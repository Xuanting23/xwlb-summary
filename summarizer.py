...
1. **top_news**: 国内要闻。原文中 [S0], [S1], [S2]... 每个编号对应一条独立的新闻。请把「联播快讯」之前的每一条国内新闻都单独列为一条，一条对应一个 JSON 对象。严禁将多条新闻合并成一条。按播出顺序排列。importance 为 1-5 星。
2. **domestic_briefs**: 国内「联播快讯」板块。该板块下的每条快讯分别是一条，逐一列出，严禁合并。
3. **international_news**: 国际新闻。原文中每条国际新闻分别是一条，逐一列出，严禁将多条国际新闻合并成一条。
4. **international_briefs**: 国际快讯。每条快讯分别是一条，逐一列出，严禁合并。
5. **keywords**: 5 个最具代表性的关键词。
6. **segment_id（极其重要）**: 每条摘要必须填写对应的分段编号 [S0][S1]... 原文中每个 [SX] 是一条独立的新闻，一条摘要只对应一个分段编号，不要一条摘要对应多个分段。
7. **严禁合并**: 四个板块中，每个 JSON 对象只描述一条新闻 / 一条快讯。绝对不允许把多条新闻合并到一个对象里。

...
1. **top_news**: 国内要闻。每个 [SX] 编号对应一条独立的新闻，一条对应一个 JSON 对象，严禁合并。按播出顺序排列。importance 为 1-5 星。
2. **domestic_briefs**: 国内「联播快讯」。该板块会将多条快讯放在同一个分段中，请根据标题变化和话题切换将其拆分为独立的条目，逐一列出，严禁合并。
3. **international_news**: 国际新闻。注意：国际新闻经常被合并在同一个分段中，请仔细阅读内容，根据话题切换、国家/地区变化、事件变化将其拆分为独立条目。例如「A 国称…… B 国计划……」应拆为两条。逐一列出，严禁合并。
4. **international_briefs**: 国际快讯。同 domestic_briefs，多条快讯常在一个分段中，必须根据标题和话题变化拆分为独立条目。

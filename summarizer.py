...
1. **top_news**: 国内要闻。请仔细阅读原文，把每一条重要的国内新闻都单独列出（除去末尾的「联播快讯」部分），不要遗漏任何一条。按新闻联播的播出顺序排列。importance 为 1-5 星。
2. **domestic_briefs**: 国内「联播快讯」。把原文中「联播快讯」板块的每一条快讯都单独列出，不要合并，不要遗漏。
3. **international_news**: 国际新闻。把每一条国际新闻都单独列出，不要遗漏。
4. **international_briefs**: 国际快讯。把每一条国际快讯都单独列出，不要遗漏。
5. **keywords**: 5 个最具代表性的关键词。
6. **segment_id（极其重要）**: 每条摘要必须填写对应的分段编号 [S0][S1]... 务必准确对应！如果一条摘要整合了多条分段的内容，填最主要的那条编号。
7. **数量说明**: 以上各个板块的条目数量完全取决于当天新闻联播的实际内容，不要自行增减。有几条就写几条。
